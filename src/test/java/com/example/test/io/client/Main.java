...
public class Main {
	public static void main(String[] args) {
		...
		// 사용하지 않는 connection을 pool에서 제거하는 시간기준
		config.timeBetweenEvictionRunsMillis = -1;

		// 놀고 있는 connection의 제거 여부
		config.testWhileIdle = false;
		NetworkConfig networkConfig = new NetworkConfig()
				.setConnectTimeout(5 * 1000).setReadTimeout(5 * 1000)
				.setSendBufferSize(1024).setSoLinger(0)
				.setReceiveBufferSize(1024).setTcpNoDelay(true)
				.setReuseAddress(false).setKeepAlive(false);
		//
		ConnectionPoolFactory poolFactory = new ConnectionPoolFactory(
				networkConfig, config, "127.0.0.1", 8080);
		ExecutorService executorService = Executors
				.newCachedThreadPool(new ThreadFactoryUtils("IO", false, 5));
