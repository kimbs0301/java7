...
public class ConnectionPoolFactory {
	...
	public ConnectionPoolFactory(NetworkConfig networkConfig, Config config,
			String ip, int port) {
		this.address = new InetSocketAddress(ip, port);
		this.networkConfig = networkConfig;
		ConnectionFactory factory = new ConnectionFactory(networkConfig, ip,
				port);
	...
	public IoSocket getConnection() throws Exception {
		...
		if (ioSocket == null || ioSocket.isClosed() == true) {
			...
			socket.setSoLinger(networkConfig.getSoLinger() > 0,
					networkConfig.getSoLinger());
