...
public class TimeCheckedUtils {
	...
	public String getCheckedEndStr(String name) {
		...
		if (sec == 0L) {
			...
			sb.append("0sec ").append(mis).append("mis ").append(nanos)
					.append("nanos");
		} else {
			...
			sb.append(sec).append("sec ").append(mis).append("mis ")
					.append(nanos).append("nanos");
		}
		return sb.toString();
	}
	...
	public long[] getCheckedEnd() {
		...
		if (sec == 0L) {
			...
			time[2] = runNanoTime
					- (time[0] * 1000000000L + time[1] * 1000000L);
		}
		return time;
	}
	...
	public static String getTotalStr(long runTotalNanoTime) {
		...
		if (second == 0L) {
			sb.append("0sec ")
					.append(TimeUnit.NANOSECONDS.toMillis(runTotalNanoTime))
					.append("mis");
		} else {
			long n = runTotalNanoTime - (1000000000L * second);
			sb.append(second).append("sec ")
					.append(TimeUnit.NANOSECONDS.toMillis(n)).append("mis");
		}
		return sb.toString();
	}
